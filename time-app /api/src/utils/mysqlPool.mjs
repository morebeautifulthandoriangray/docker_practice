...
const pool = mysql.createPool({
  connectionLimit: 100,
  // сервис бд, который используется в docker-compose.yml
  host: MYSQL_HOST,
  port: MYSQL_PORT,
  user: MYSQL_USER,
  // этот пароль мы укажем для контейнера в бд, используя перемены среды docker-compose
  // и этот пароль должен совпадать с этим паролем в приложении
